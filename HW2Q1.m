...
for ind = 1:length(DType)
    D.(DType{ind}).x = zeros(dimension, D.(DType{ind}).N); % 初始化数据
    ...
    plot(D.(DType{ind}).x(1, ~D.(DType{ind}).labels), ...
         D.(DType{ind}).x(2, ~D.(DType{ind}).labels), 'b.', 'DisplayName', 'Class 0');
    hold all;
    plot(D.(DType{ind}).x(1, D.(DType{ind}).labels), ...
         D.(DType{ind}).x(2, D.(DType{ind}).labels), 'r.', 'DisplayName', 'Class 1');
    grid on;
    xlabel('x1');
    ylabel('x2');
...
function plotERMContours(x, alpha0, mu0, Sigma0, alpha1, mu1, Sigma1, logGamma_ideal)
    ...
    lgd=legend('Correct decisions for data from Class 0', 'Wrong decisions for data from Class 0', 'Wrong decisions for data from Class 1', 'Correct decisions for data from Class 1', 'Equilevel contours of the discriminant function');
    
    set(lgd, 'FontSize', 6); % 将图例的字体大小设置为12
end
